...
class ViT(nn.Module):
    ...
    def __init__(self, patch_dim, d_model, d_ff, num_heads, num_layers, num_patches, num_classes, device = 'cuda'):
        ...
        self.cls_token = nn.Parameter(torch.randn(1,1,self.d_model)) # TODO (learnable [CLS] token embedding)
    ...
    def patchify(self, images):
        ...
        num_patches = (H * W) // (self.patch_dim ** 2)
        patches_1 = images.unfold(2, self.patch_dim, self.patch_dim)
        patches_2 = patches_1.unfold(3, self.patch_dim, self.patch_dim).reshape(N, num_patches, -1)
        return patches_2
