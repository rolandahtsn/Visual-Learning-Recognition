...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        if attn_mask is not None:
            ...
            additive_mask = attn_mask.unsqueeze(1)
            dot_product += additive_mask
        
        # apply softmax
        softmax = nn.Softmax(dim=-1)(dot_product)

        # , dropout
        drop_out = self.dropout(softmax)

        # , and use value
        y = torch.bmm(drop_out, value)
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = self.query_proj(query)  
        query = query.view(N, S, self.num_heads, -1)  
        query = query.transpose(1, 2) 

        key = self.key_proj(key)  
        key = key.view(N, T, self.num_heads, -1)  
        key = key.transpose(1, 2)  

        value = self.value_proj(value)
        value = value.view(N, T, self.num_heads, -1) 
        ...
        dot_product = dot_product / math.sqrt(self.embed_dim / self.num_heads)

        if attn_mask is not None:
            # convert att_mask which is multiplicative, to an additive mask
            # Hint : If mask[i,j] = 0, we want softmax(QKT[i,j] + additive_mask[i,j]) to be 0
            # Think about what inputs make softmax 0.
            additive_mask = dot_product.masked_fill(attn_mask==0, float("-inf"))
            dot_product += additive_mask
        ...
        output = y.transpose(1, 2).reshape(N, S, -1)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        pos_indices = torch.arange(x.shape[1], device=x.device).unsqueeze(0)
        pos_encoding = self.encoding(pos_indices) 
...
class SelfAttentionBlock(nn.Module):
    ...
    def forward(self, seq, mask):
        ...
        output = self.self_attn(seq,seq,seq,mask)
        dropout = self.dropout(output)
        residual = seq + dropout
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, seq, cond):
        ...
        # Then add a residual connection to the original input, and finally apply normalization. #############################
        output = self.cross_attn(seq,cond,cond)
        dropout = self.dropout(output)
        residual = seq + dropout
...
class TransformerDecoder(nn.Module):
    ...
    def get_causal_mask(self, _len):
        ...
        mask = torch.tril(torch.ones(_len, _len, device=self.device), diagonal=0)
