...
def vae_loss(model, x, beta = 1):
    ...
    decoder_output = model.decoder(mu + torch.exp(0.5 * log_var) * torch.randn_like(mu))
  
    recon_loss = F.mse_loss(decoder_output, x, reduction='sum') / x.size(0)
 
    kl_loss = -0.5 * torch.sum(1 + log_var - mu.pow(2) - log_var.exp())
    kl_loss /= x.size(0)  # Normalizing by batch size
...
def linear_beta_scheduler(max_epochs=None, target_val = 1):
    ...
    ##################################################################
    def _helper(epoch):
        epoch_val = epoch*target_val/max_epochs
        return epoch_val
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
