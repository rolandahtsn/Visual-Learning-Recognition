...
class DiffusionModel(nn.Module):
    def __init__(
        ...
        self.alphas_cumprod_prev = torch.cat([torch.tensor([1.], device=self.device), self.alphas_cumprod[:-1]], dim=0)

        ##################################################################
        # TODO 3.1: Pre-compute values needed for forward process.
        ##################################################################
        # This is the coefficient of x_t when predicting x_0
        self.x_0_pred_coef_1 = torch.reciprocal(torch.sqrt(self.alphas_cumprod))
        # This is the coefficient of pred_noise when predicting x_0
        self.x_0_pred_coef_2 = torch.sqrt(torch.reciprocal(self.alphas_cumprod) - 1.) / torch.sqrt(torch.reciprocal(self.alphas_cumprod))

        ##################################################################
        # TODO 3.1: Compute the coefficients for the mean.
        ##################################################################
        # This is coefficient of x_0 in the DDPM section
        val_1 = 1.-self.alphas_cumprod
        val_2 = 1. - self.alphas_cumprod_prev
        self.posterior_mean_coef1 = torch.sqrt(torch.mul(self.alphas_cumprod_prev, self.betas) /(val_1))
        # This is coefficient of x_t in the DDPM section
        self.posterior_mean_coef2 = torch.sqrt(torch.mul(alphas, val_2) /(val_1))

        ##################################################################
        # TODO 3.1: Compute posterior variance.
        ##################################################################
        self.posterior_variance = self.betas * val_2 / (val_1)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
    ...
    def model_predictions(self, x_t, t):
        ...
        alphas_sqrt = torch.sqrt(1. / self.alphas_cumprod)
        alphas_sqrt_minus_one = torch.sqrt(1. / self.alphas_cumprod - 1.)
        x_0 = extract(alphas_sqrt, t, x_t.shape) * x_t - extract(alphas_sqrt_minus_one, t, x_t.shape) * pred_noise
        x_0 = torch.clamp(x_0, -1, 1, out=x_0)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
    ...
    def predict_denoised_at_prev_timestep(self, x, t: int):
        ...
        pred_img = mean + torch.sqrt(var) * z
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
        return pred_img, x_0
    ...
    def ddim_step(self, batch, device, tau_i, tau_isub1, img, model_predictions, alphas_cumprod, eta):
        ...
        tau_i = torch.tensor([tau_i], device = self.device)
        tau_isub1 = torch.tensor([tau_isub1], device = self.device)
        ...
        alpha_tau = self.alphas_cumprod[tau_i]
        alpha_tau1 = self.alphas_cumprod[tau_isub1]

        # Step 3: Compute \sigma_{\tau_{i}}
        sigma_2 = eta*(1.-alpha_tau1)/(1.-alpha_tau) * (1.-alpha_isub1)
        sigma_1 = torch.sqrt(sigma_2)

        # Step 4: Compute the coefficient of \epsilon_{\tau_{i}}
        noise_c = torch.sqrt(torch.clamp(1. - alpha_tau1 - sigma_2, min=0))

        # Step 5: Sample from q(x_{\tau_{i - 1}} | x_{\tau_t}, x_0)
        # HINT: Use the reparameterization trick
        if tau_i > 0:
            z = torch.randn_like(img)
            img = torch.sqrt(alpha_tau1) * x_0 + noise_c * pred_noise + sigma_1 * z
        else:
            img = torch.sqrt(alpha_tau1) * x_0 + noise_c * pred_noise
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
