    ...
    while iters < num_iterations:
        for train_batch in train_loader:
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                ...
                discrim_fake = (disc(gen_output.detach()))
                ##################################################################
                #                          END OF YOUR CODE                      #
                ##################################################################
                ...
                # discriminator on it.
                ###################################################################
                rand_val = torch.rand(gen_output.shape).cuda()
                interp = (rand_val * train_batch + (1 - rand_val) * gen_output.detach()).requires_grad_(True)
            ...
            if iters % 5 == 0:
                with torch.cuda.amp.autocast(enabled=amp_enabled):
                    ...
                    generator_loss = gen_loss_fn(discrim_fake.float())
            ...
            if iters % log_period == 0 and iters != 0:
                with torch.no_grad():
                    with torch.cuda.amp.autocast(enabled=amp_enabled):
                        ...
                        ##################################################################
                        gen_output = gen(100)
                        generated_samples = (gen_output + 1)/2
                        ##################################################################
                        #                          END OF YOUR CODE                      #
                        ##################################################################
                    save_image(
