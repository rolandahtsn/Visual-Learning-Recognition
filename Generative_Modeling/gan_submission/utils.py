...
def interpolate_latent_space(gen, path):
    ...
    ##################################################################
    samples  = torch.zeros((100,128), dtype =  torch.float32)
    indices = torch.arange(100)
    i_values = torch.lerp(torch.tensor(-1.0), torch.tensor(1.0), indices % 10 / 9)
    j_values = torch.lerp(torch.tensor(-1.0), torch.tensor(1.0), indices // 10 / 9) 

    samples[:, 0] = i_values
    samples[:, 1] = j_values

    latent_space_img = gen.forward_given_samples(samples)
    torchvision.utils.save_image(latent_space_img.float(), path, nrow=10)
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
