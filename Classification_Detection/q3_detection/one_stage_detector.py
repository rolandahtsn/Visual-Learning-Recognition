...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        fpn_params = OrderedDict()

        for name, in_channels in input_channels.items():
            fpn_params[name] = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0)

        fpn_params['output_p3'] = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1)
        fpn_params['output_p4'] = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1)
        fpn_params['output_p5'] = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1)
        self.fpn_params = nn.ModuleDict(fpn_params)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images: torch.Tensor):
        ...
        c5_features = backbone_feats["c5"]
        c5_backbone = self.fpn_params["lateral_c5"](c5_features)
        output_p5 = self.fpn_params["output_p5"](c5_backbone)

        c4_features = backbone_feats["c4"]
        c4_backbone = self.fpn_params["lateral_c4"](c4_features)
        upsampled_p5 = F.interpolate(output_p5, scale_factor=2)
        p4_mergedfeatures = c4_backbone + upsampled_p5
        output_p4 = self.fpn_params["output_p4"](p4_mergedfeatures)

        c3_features = backbone_feats["c3"]
        c3_backbone = self.fpn_params["lateral_c3"](c3_features)
        upsampled_p4 = F.interpolate(output_p4, scale_factor=2)
        p3_mergedfeatures = c3_backbone + upsampled_p4
        output_p3 = self.fpn_params["output_p3"](p3_mergedfeatures)

        # Lateral connections
        fpn_feats["p3"] = output_p3
        fpn_feats["p4"] = output_p4
        fpn_feats["p5"] = output_p5
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        stem_cls.append(nn.Conv2d(in_channels= in_channels, out_channels=stem_channels[0], kernel_size=3, padding=1))
        stem_cls.append(nn.ReLU())
        stem_box.append(nn.Conv2d(in_channels= in_channels, out_channels=stem_channels[0], kernel_size=3, padding=1))
        stem_box.append(nn.ReLU())
        for i in range (1, len(stem_channels)):
            stem_cls.append(nn.Conv2d(in_channels= stem_channels[i-1], out_channels=stem_channels[i], kernel_size=3, padding=1))
            stem_cls.append(nn.ReLU())
            stem_box.append(nn.Conv2d(in_channels= stem_channels[i-1], out_channels=stem_channels[i], kernel_size=3, padding=1))
            stem_box.append(nn.ReLU())
        ...
        self.pred_cls = nn.Conv2d(stem_channels[-1], out_channels=num_classes, kernel_size=3, stride=1, padding=1)  
        self.pred_box = nn.Conv2d(stem_channels[-1], out_channels=4, kernel_size=3, stride=1, padding=1) 
        self.pred_ctr = nn.Conv2d(stem_channels[-1], out_channels=1, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level, feat in feats_per_fpn_level.items():
            cls_output = self.stem_cls(feat)
            box_output = self.stem_box(feat)

            class_logits[level] = self.pred_cls(cls_output).reshape(feat.shape[0], -1, self.num_classes)
            boxreg_deltas[level] = self.pred_box(box_output).reshape(feat.shape[0], -1, 4)
            centerness_logits[level] = self.pred_ctr(box_output).reshape(feat.shape[0], -1, 1) 
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        ######################################################################
        # Feel free to delete this line: (but keep variable names same)
        # Calculate shapes of feature maps for each FPN level
        fpn_feats_shapes = {level_name: feat.shape for level_name, feat in map(lambda x: (x[0], x[1]), backbone_feats.items())}
        strides_per_fpn = self.backbone.fpn_strides
        locations_per_fpn_level = get_fpn_location_coords(fpn_feats_shapes, strides_per_fpn, device=images.device)
        ...
        matched_gt_deltas = [
            {level: fcos_get_deltas_from_locations(locations_per_fpn_level[level], boxes, self.backbone.fpn_strides[level]) 
        ...
        ######################################################################
        # Feel free to delete this line: (but keep variable names same)
        classes = matched_gt_boxes[:, :, -1].long()
        temp_classes = F.one_hot(classes + 1, self.num_classes + 1)
        target_cls_logits = temp_classes[:, :, 1:]

        pred_boxreg_deltas = pred_boxreg_deltas.view(-1, 4).to("cuda")
        matched_gt_deltas = matched_gt_deltas.view(-1, 4).to("cuda")

        gt_centerness = fcos_make_centerness_targets(matched_gt_deltas)
        pred_ctr_logits = pred_ctr_logits.view(-1,)
        target_ctr_logits = gt_centerness

        loss_cls = sigmoid_focal_loss(inputs=pred_cls_logits, targets=target_cls_logits.float())
        loss_box = 0.25 * F.l1_loss(pred_boxreg_deltas, matched_gt_deltas, reduction="none")
        loss_ctr = F.binary_cross_entropy_with_logits(pred_ctr_logits, target_ctr_logits, reduction="none")

        loss_box[matched_gt_deltas < 0] *= 0.0
        loss_ctr[target_ctr_logits < 0] *= 0.0
