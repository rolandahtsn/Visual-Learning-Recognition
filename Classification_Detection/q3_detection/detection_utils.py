...
def fcos_get_deltas_from_locations(
    ...
    deltas[:, 2:] = (gt_boxes[:, 2:4] - locations[:, :2]) / stride

    invalid_boxes = gt_boxes[:, :4] == -1
    deltas[invalid_boxes] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    deltas = torch.clamp(deltas, min=0).to(device)
    locations = locations.to(device)
    
    box_1= locations[:, 0] - deltas[:, 0] * stride
    boy_1= locations[:, 1] - deltas[:, 1] * stride
    box_2= locations[:, 0] + deltas[:, 2] * stride
    boy_2= locations[:, 1] + deltas[:, 3] * stride
    output_boxes = torch.stack((box_1,boy_1, box_2, boy_2), dim = 1)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = torch.sqrt(
        (torch.minimum(deltas[:,0], deltas[:,2])*torch.minimum(deltas[:,1], deltas[:,3]))/
        (torch.maximum(deltas[:,0], deltas[:,2])*torch.maximum(deltas[:,1], deltas[:,3]))
        )
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        B, C, H, W = feat_shape
        r_coords = torch.arange(0, H) * level_stride + level_stride / 2
        c_coords = torch.arange(0, W) * level_stride + level_stride / 2
        grid_r, grid_c = torch.meshgrid(r_coords, c_coords)
        coordinates_array = torch.stack((grid_r.flatten(), grid_c.flatten()), dim=1)

        coordinates_array = coordinates_array.to(device)
        location_coords[level_name] = coordinates_array
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
